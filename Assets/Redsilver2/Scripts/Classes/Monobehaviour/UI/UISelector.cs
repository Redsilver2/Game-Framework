...
namespace RedSilver2.Framework.UI
{
    public class UISelector : MonoBehaviour
    {
        ...
        public uint MaxVerticalIndex => selections != null ? (uint)(selections.Count - 1) : 1;
        ...
        public void AddColumn(int row, UISelection selection)
        {
            ...
            if (!selections[row].Contains(selection)) {
                selections[row]?.Add(selection);
                selection?.SetOwner(this);
            }
        }
        ...
        private List<List<UISelection>> GetUISelections()
        {
            ...
            foreach (UISelectorData data in datas)
            {
                UISelection[] selections = data.Column;
